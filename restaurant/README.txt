Restaurant Midokura Code Testing
...
The source code of the solution is at src/restaurant and for running, there are JUnit tests at src/restaurant/test.

NOTE: IntelliJ or Eclipse can be used for running the tests and also Apache Ant with the file build.xml

* Time and Space

The memory space used for this solution is in order of the size of tables to
assign. The more tables, the more time to assign seats. Also the time is in
order of the number of customer groups to seat.

The memory space of this solution is in order of the size of the number of
tables, and the number of groups.

The presented solution has basic thread safe. For using in a heavy parallel
processing a better algorithm is needed, but maybe best do it in Scala than in
Java.
